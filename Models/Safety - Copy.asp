...
Class SafetyHelper
    ...
    Public Function DepartmentList(ByVal StreamType, ByVal DateID)
        ...
        Dim List : Set List = Server.CreateObject("Scripting.Dictionary")
        Dim SQLStr : SQLStr = "SELECT dateid , departmentid, department, accidentcnt, nearaccidentcnt, incidentcnt, agid FROM vwTierSafety WHERE dateid='" & DateID & "'" & IIf(streamtype="all",""," AND streamtype='" & StreamType & "'") & " ORDER BY dateid desc, department, departmentid"
        Dim Rs : Set Rs = DbExecute(SQLStr)
        Dim Item
        Dim pItem
        ...
        Do While Not Rs.Eof
            Set Item = List(CInt(IIf(streamtype="all",Rs("agid"),Rs("departmentid"))))
            Item.DateID = Rs("dateid")
            Item.AccidentCnt = Rs("accidentcnt")
            Item.NearAccidentCnt = Rs("nearaccidentcnt")
            Item.IncidentCnt = Rs("incidentcnt")
