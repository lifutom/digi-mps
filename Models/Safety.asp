...
Class Safety
    ...
    Public Property Let NoInitSafetyID(Value)
        mID = Value
    End Property
    ...
    Public Function Save()
        ...
        Cmd.Parameters.Append Parameter
        Parameter.Value = IIf(NearAccidentCnt="",0,NearAccidentCnt)
...
Class SafetyHelper
    ...
    Private Sub Class_Initialize()
          ...
          SQLNearMiss = "WITH x As (" & _
                        "SELECT dateid, departmentid, agid, agname,  department, streamtype, area, yeardate, kw, 0 As accidentcnt, CASE WHEN dateid <'" & DigiSaftyStartDate & "' THEN 0 ELSE nearaccidentcnt END As nearaccidentcnt, 0 As incidentcnt " & _
                        "FROM vwGoodCatchesDateDep " & _
                        "UNION ALL " & _
                        "SELECT dateid, departmentid, agid, agname, department, streamtype, area, yeardate, kw, accidentcnt, CASE WHEN dateid >='" & DigiSaftyStartDate & "' THEN 0 ELSE nearaccidentcnt END As nearaccidentcnt, incidentcnt " & _
                        "FROM vwTierSafety " & _
                        ") "
    ...
    Public Function SafetyList
        ...
        Do While Not Rs.Eof
            Set Item = New Safety
            Item.NoInitSafetyID = Rs("safetyid")
    ...
    Private Function Tier2ChartData (ByVal DateID, ByVal StreamType)
        ...
        Dim SQLStr : SQLStr = SQLNearMiss &  "SELECT dateid, SUM(accidentcnt) As accidentcnt, SUM(nearaccidentcnt) As nearaccidentcnt, SUM(incidentcnt) As incidentcnt FROM x WHERE streamtype='" & StreamType & "' AND CONVERT(date,dateid) BETWEEN DATEADD(month,-1,CONVERT(date,dateid)) AND CONVERT(date,'" & DateID & "') GROUP BY dateid ORDER BY dateid"
        ...
        With oJSON.Data
            ...
            With .Item("datasets")
                ...
                .Add idx,oJSON.Collection()
                With .Item(idx)
                    .Add "label",Html.Encode("Good Catch")
                    .Add "data",oJSON.Collection()
                    With .Item("data")
                        idx1=1
                        For Each Item In List.Items
    ...
    Private Function Tier2HistChartData (ByVal DateID, ByVal StreamType)
        ...
        Dim SQLStr : SQLStr = SQLNearMiss &  "SELECT yeardate, kw, SUM(accidentcnt) As accidentcnt, SUM(nearaccidentcnt) As nearaccidentcnt, SUM(incidentcnt) As incidentcnt FROM x WHERE streamtype='" & StreamType & "' AND CONVERT(date,dateid) BETWEEN DATEADD(year,-1,CONVERT(date,dateid)) AND CONVERT(date,'" & DateID & "') GROUP BY yeardate, kw ORDER BY yeardate, kw"
    ...
    Public Function DepartmentList(ByVal StreamType, ByVal DateID)
        ...
        If StreamType="all" Then
            Set DepList = oDepartment.ListAll()
        Else
            Set DepList = oDepartment.ListByStreamType(StreamType)
        End If
        ...
        For Each pItem In DepList.Items
            ...
            List.Add CInt(Item.DepartmentID), Item
        Next

        Dim SQLStr : SQLStr = SQLNearMiss &  "SELECT dateid , departmentid, department, agid, SUM(accidentcnt) As accidentcnt , SUM(nearaccidentcnt) As nearaccidentcnt, SUM(incidentcnt) As incidentcnt FROM x WHERE dateid='" & DateID & "'" & IIf(streamtype="all",""," AND streamtype='" & StreamType & "'") & " GROUP BY dateid , departmentid, department, agid ORDER BY dateid desc, department, departmentid"
        Dim Rs : Set Rs = DbExecute(SQLStr)
       
        Do While Not Rs.Eof

            Set Item = List(CInt(IIf(streamtype="all",Rs("departmentid"),Rs("departmentid"))))
            Item.DateID = Rs("dateid")
            Item.AccidentCnt = Rs("accidentcnt")
            Item.NearAccidentCnt = Rs("nearaccidentcnt")
            Item.IncidentCnt = Rs("incidentcnt")
    ...
    Private Function Tier3ChartData (ByVal DateID)
        ...
        Dim SQLStr : SQLStr = SQLNearMiss & "SELECT agid As departmentid, agname As department, SUM(accidentcnt) As accidentcnt, SUM(nearaccidentcnt) As nearaccidentcnt, SUM(incidentcnt) As incidentcnt FROM x WHERE CONVERT(date,dateid) = CONVERT(date,'" & DateID & "') GROUP BY agid, agname ORDER BY agname"
    ...
    Private Function Tier3IncChartData (ByVal DateID)
        ...
        With oJSON.Data
            ...
            With .Item("datasets")
                .Add idx,oJSON.Collection()
                With .Item(idx)
                    .Add "label","Safety Incidents (" & DateID  & ")"
                    .Add "data",oJSON.Collection()
                    With .Item("data")
                        idx1=1
                        For Each Item In List.Items
    ...
    End Function
     ...
     Private Function Tier3NearChartData (ByVal DateID)
        ...
        Dim SQLStr : SQLStr = SQLNearMiss & "SELECT yeardate, MONTH(dateid) As monthdate, SUM(accidentcnt) As accidentcnt, SUM(nearaccidentcnt) As nearaccidentcnt, SUM(incidentcnt) As incidentcnt FROM x WHERE yeardate = YEAR(CONVERT(date,'" & DateID & "')) GROUP BY yeardate, MONTH(dateid) ORDER BY yeardate, MONTH(dateid)"
