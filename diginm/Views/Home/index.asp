...
<section class="mb-5">
    ...
    <form action="<%=CurRootFile%>/home/TasksPost" method="POST" enctype="multipart/form-data">
    ...
    <div class="card card-cascade narrower">
        ...
        <div class="view view-cascade py-3 gradient-card-header grey mx-4 d-flex justify-content-between align-items-center">
            <a href="" class="white-text text-center mx-3"><i class="fas fa-home"></i>&nbsp;Meldung&nbsp;<%=IIf(Session("login") <> "", "(user: " & Session("login"),"")%>)</a>
        </div>
        <!--/Card header-->
            ...
            <div class="row">
                <div class="col-4">
                     <div class="card">
                          ...
                          <div class="card-body">
                               <input type="date" name="nmdate" id="nmdate" value="<%=DBFormatDate(vwObject.NearDate)%>" required/>
                          </div>
                     </div>
                </div>
                <div class="col-3">
                     <div class="card">
                          ...
                          <div class="card-body">
                               <input type="time" id="nmtime" name="nmtime" value="<%=Left(CStr(vwObject.NearTime),5)%>" pattern="([1]?[0-9]|2[0-3]):[0-5][0-9]" required/>
                          </div>
                     </div>
                </div>
                <div class="col-5">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-2">
                                    ...
                                    <select name="buildingid" id="buildingid" class="search-drowdown form-control" style="width: 100%">
                                        ...
                                        <% For Each vwItem In vwBuildingList.Items %>
                                            <option value="<%=vwItem.Value%>" <%=IIf(CInt(vwItem.Value) = CInt(IIf(vwObject.BuildingID <> "", vwObject.BuildingID, -1)),"selected","")%>><%=vwItem.Name%></option>
                                        <% Next %>
                                    </select>
                                </div>
                                <select name="regionid" id="regionid" class="search-drowdown form-control" style="width: 100%">
                                    ...
                                    <% For Each vwItem In vwRegionList.Items %>
                                        <option value="<%=vwItem.Value%>" <%=IIf(CInt(vwItem.Value) = CInt(IIf(vwObject.RegionID <> "",vwObject.RegionID, CInt(-1))),"selected","")%>><%=vwItem.Name%></option>
                                    <% Next %>
                                </select>
                        ...
                        <div class="row">
                            <div class="col-6">
                                <div class="form-group shadow-textarea">
                                        ...
                                        <textarea class="form-control z-depth-1" id="description" name="description" rows="9" placeholder="Beschreibung des Vorfalls..." required><%=vwObject.Description%></textarea>
                                </div>
                                <div>
                                    <a class="btn btn-default" href="<%=curRootFile & "/home/logout"%>">Logout</a>&nbsp;&nbsp;
                                    <input class="btn btn-primary" type="submit" name="cmdnext" id="cmdnext" value="Weiter">
                                </div>
                            </div>
                            <div class="col-6">
                                <label for="nearimage">Foto</label>
                                 <div class="file-upload-wrapper">
                                        ...
                                        data-height="300"
                                        data-default-file="<%=IIf(vwObject.VirtualPath <> "",curRootFile & "/" & vwObject.VirtualPath, "")%>"/>
                                 </div>
                            </div>
                        </div>
                    </div>
...
<!-- javascript -->
<script src="<%=curRootFile%>/_js/home/index.js?v.1.3"></script>
