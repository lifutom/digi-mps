...
$(function () {
    ...
    $('#accessitemid').change(function() {
        setAccessRights($('#accessitemid').val());
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data["status"] == "OK") {
                ...
                var accessitemdropdown = $('#accessitemid');

                accessitemdropdown.empty();
                $.each(data['accessitem'], function (key, entry) {
                    accessitemdropdown.append($('<option></option>').attr('value', entry.value).text(entry.name));
                });
                accessitemdropdown.prop('selectedIndex', 0);

                rightdropdown.empty();
                $.each(data['accessright'], function (key, entry) {
                    rightdropdown.append($('<option></option>').attr('value', entry.value).text(entry.name));
                });
                accessitemdropdown.val(data['accessitemid']);
                if (data['accesstypeid'] == 3) {
                   accessitemdropdown.prop("disabled",false);
                } else {
                   accessitemdropdown.prop("disabled",true);
                }
                rightdropdown.val(data['accessrightid']);
                rightdropdown.prop("disabled",true);
            } else {
...
function setAccessRights(id) {

    var data = {
        list: 'accessright-by-id',
        id: id
    }
