...
class cExtendedHTMLHelper
    ...
    public Function DropDownListItem (elId, elValue, list , idName, valueName, attribs, myclass, action, withempty)
		Dim resStr, lisItem
		resStr= "<select id='" + elID + "' name='" + elID + "' class='" & myclass & "' " & action & ">"
        If WithEmpty Then
           resStr = resStr + "<option value=''>" + Encode("--" & GetLable("selKeineAuswahl",Session("lang"))) & "--</option>"
        End If
